...
const ProductForm = ({ initialData = null, onSubmit, onClose, loading }) => {
  ...
  const validate = () => {
    ...
    if (form.imageUrl && !/^https?:\/\/.+/.test(form.imageUrl)) {
      newErrors.imageUrl = 'Enter a valid URL (starts with http/https)';
      isValid = false;
    }
  ...
  // ── Field component ─────────────────────────────────────────────────────────
  const Field = ({ icon: Icon, label, name, type = 'text', placeholder, isTextarea }) => (
    <div>
      ...
      {isTextarea ? (
        <textarea
          ...
          rows={4}
          className={`input-field resize-none ${errors[name] ? 'border-red-400 focus:ring-red-400/30 focus:border-red-400' : ''}`}
        />
      ) : (
        <input
          type={type}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="card w-full max-w-lg animate-scale-in max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-5 overflow-y-auto">
          <Field icon={FiPackage} label="Product Name" name="name" placeholder="e.g. Wireless Headphones" />
          <Field icon={FiDollarSign} label="Price (USD)" name="price" type="number" placeholder="e.g. 49.99" />
          <Field icon={FiImage} label="Image URL (optional)" name="imageUrl" placeholder="https://example.com/image.jpg" />
